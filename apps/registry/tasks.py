...
def nightly_integrity_sweep():
    """Recompute SHA-256 for 100% of active credentials. EVS-N05.

    Flags tampered records as SecurityEvents and triggers quarantine.
    """
    ...
    while True:
        batch = list(
            Credential.objects.filter(status=Credential.STATUS_ACTIVE)
